...
/**
 * If a user writes a snippet that returns `TestResult`, then we can collect all `TestResult` and
 * create a test-report from it.
 *
...
class TestEnviroment {
  ...
  fun testReport(): String? =
    buffer.get()?.takeIf { it.isNotEmpty() }?.run {
      """
  ...
  fun insert(result: List<TestResult>): List<TestResult> = buffer.updateAndGet { it + result }
}
